...
_.extend(Server.prototype, {
  run: function(script, cb) {
    ...
    server.scp(script, remoteScript, function (err) {
      if ((err && !_.has(err, 'code')) || (err && _.has(err, 'code') && err.code !== 0)) {
        return cb(err);
      }
      server.ssh('chmod 755 '+remoteScript+' && '+remoteScript+' && rm '+remoteScript, function(err, stdout) {
        if ((err && !_.has(err, 'code')) || (err && _.has(err, 'code') && err.code !== 0)) {
          return cb(err);
        }
        cb(null, stdout);
