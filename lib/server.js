...
_.extend(Server.prototype, {
  ...
  ssh: function(command, cb) {
    var args = _.union(this.formatIdentity(this.options), ['-o', 'StrictHostKeyChecking=no'], this.formatDestination(this.options));
    args.push("''" + command + "''");
  ...
  scp: function(local, remote, cb) {
    ...
    fs.exists(local, function (exists) {
      ...
      var args =  _.union(server.formatIdentity(server.options), ['-o', 'StrictHostKeyChecking=no', local, destination]);
